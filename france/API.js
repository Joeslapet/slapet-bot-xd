...
export async function getRandomJoke() {
  ...
  if (!response.ok) throw new Erreur('Network response was not ok.');
...
export async function getRandomTrivia() {
  ...
  if (!response.ok) throw new Erreur(`Invalid API response: ${response.status}`);
  const result = await response.json();
  if (!result.results || !result.results[0]) throw new Erreur('No trivia data received.');
...
export async function getBibleVerse(reference) {
  ...
  if (!response.ok) throw new Erreur('Invalid reference');
  const data = await response.json();
  return data;
}
...
export async function defineTerm(term) {
  ...
  if (!data.list || data.list.length === 0) throw new Erreur('No definition found');
  return data.list[0];
}
