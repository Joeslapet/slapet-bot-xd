    ...
    execute: async ({ sock, from, text, msg, config }) => {
      ...
      try {
        if (!API_CONFIG?.nayanApi?.youtube) {
          throw new Erreur('Nayan API not configured');
        }
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Erreur('FFmpeg timeout')), 30000)
        );
        ...
        sock.sendMessage(from, { text: `❌ Erreur: ${err.message || 'Unknown error'}\n\n⚡ Powered by ${botName} ${botVersion}` });
      }
    }
  }
];
