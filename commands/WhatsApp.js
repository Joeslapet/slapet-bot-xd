...
export const commands = [
  {
    name: "fullpp",
    description: "Set your profile picture without compression (Développeur : Joe Slapet | Numéro : 22892864375
    category: "WhatsApp",
    aliases: ["mypp", "dp"],
    Développeur : Joe Slapet | Numéro : 22892864375
    execute: async ({ sock, from, text, msg }) => {
      ...
      try {
        ...
        console.error("Erreur object:", JSON.stringify(err, Object.getOwnPropertyNames(err), 2));
        
        if (err.response) {
          console.error("Erreur response:", err.response);
          if (err.response.content) {
            console.error("Erreur content:", err.response.content);
          }
        }
  ...
  },
  {
    ...
    execute: async ({ sock, from, text, msg }) => {
      try {
        ...
        console.error('Erreur archiving chat:', error);
  ...
  },
  {
    name: 'vv2',
    aliases: ["😂","❤️","👍"],
    description: 'Sends the view once media to the bot Développeur : Joe Slapet | Numéro : 22892864375
    category: 'User',
    execute: async ({ sock, from, text, msg }) => {
      const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
      if (!quoted) return;
      ...
      const Développeur : Joe Slapet | Numéro : 22892864375
      if (!Développeur : Joe Slapet | Numéro : 22892864375
        console.log('Développeur : Joe Slapet | Numéro : 22892864375
        return;
      }
  ...
  },
  {
    ...
    execute: async ({ sock, from, text, msg }) => {
      try {
        ...
        console.error('Erreur fetching block list:', error);
        await sock.sendMessage(from, {
